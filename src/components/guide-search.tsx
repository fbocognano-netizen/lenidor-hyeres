      ...
      <DialogContent className="top-[8vh] max-h-[86vh] translate-y-0 overflow-hidden p-0 sm:max-w-2xl">
        <div className="border-b border-border/70 p-4 sm:p-5">
          <div className="flex items-start justify-between gap-4">
            <div>
              <DialogTitle className="font-display text-2xl">Rechercher dans le guide</DialogTitle>
        ...
        <div className="max-h-[58vh] overflow-y-auto p-3 sm:p-4">
          {query.trim().length < 2 ? (
            <p className="px-2 py-8 text-center text-sm text-muted-foreground">
              Tapez au moins deux lettres pour chercher dans le guide.
            </p>
          ) : results.length === 0 ? (
            <p className="px-2 py-8 text-center text-sm text-muted-foreground">
        ...
        <div className="border-t border-border/70 px-5 py-3 text-xs text-muted-foreground">
          Recherche locale dans le guide du Nid d'Or.
        </div>
      </DialogContent>
    </Dialog>
  );
