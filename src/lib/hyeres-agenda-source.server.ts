...
export type CityAgendaCard = { sourceUrl: string; title: string };
...
export type CityAgendaEvent = {
  ...
  travelerCategory: AgendaCategory;
...
async function fetchCityText(path: string, searchParams: URLSearchParams): Promise<string> {
  ...
  try {
    ...
    if (body.length > MAX_RESPONSE_BYTES) throw new Error("La réponse de la Ville est trop volumineuse.");
    return body;
  } finally {
...
export async function getCityAgendaEvents(): Promise<CityAgendaEvent[]> {
  ...
  for (let page = 1; page <= 20; page += 1) {
    const body = await fetchCityText(CITY_EVENTS_PATH, new URLSearchParams({ per_page: "100", page: String(page) }));
  ...
  return events.flatMap((event) => {
    ...
    const category = classValue(event.class_list, "type-evenement-");
    const scheduleText = decodeHtml(event.content?.rendered ?? "") || null;
    return [{
      sourceEventId: String(event.id),
      sourceUrl,
      title,
      category,
      travelerCategory: classifyAgendaEvent({ title, sourceCategory: category, scheduleText }),
...
export async function getCityAgendaPreview(days: number, start = new Date()) {
  ...
  const [daysWithCards, events] = await Promise.all([
    Promise.all(
  ...
  return {
    ...
    days: daysWithCards,
    eventsByUrl: new Map(events.map((event) => [event.sourceUrl, event])),
  };
}
import { classifyAgendaEvent, type AgendaCategory } from "./agenda-categories";
